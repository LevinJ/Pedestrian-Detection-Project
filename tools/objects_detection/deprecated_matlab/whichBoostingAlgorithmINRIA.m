%%%%%%%%%%%%%%%%%Which boosting algorithm
    ...
    names = {'HOG-multiScale-crisp' ,'vadaBoost' ,'8000Learners', 'HOG-multiScale_8000_vada'};
    
    plotNames = {'Adaboost 2k', 'Vadaboost 2k', 'Adaboost 8k', 'Vadaboost 8k'};
 plot_colors = {colors{3}, colors{4}, colors{3},colors{4}, colors{4},colors{1},colors{8},colors{8}}

    for i=1:numel(names)
        [aps{end+1}, lgd{end+1}, handels(i)] =  addCurve(h, plot_colors{i}, names(i), plotNames(i), basefolder, resultfolder);
         if strcmp(names{i}, '8000Learners') || strcmp(names{i}, 'HOG-multiScale_8000_vada')
