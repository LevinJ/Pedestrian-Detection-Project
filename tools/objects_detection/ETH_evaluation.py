...
def plot_caltech_evaluation(results_path, disable_show, exp_idx ):
    ...
    caltech_results_dir = os.path.join(caltech_evaluation_dir, "data-ETH/res/Ours-wip_"+results_name)
    ...
    if at_visics:
        ...
        shutil.copy(os.path.join(caltech_evaluation_dir, "results", "Ours-wip.mat"), result_mat_path)
...
def main():
    ...
    parser.add_option("-e", "--exp_idx", dest="exp_idx",
                        type="int",default=1,
                       help="specify which experiment setup to use, default value is 1 %s"%exps)
