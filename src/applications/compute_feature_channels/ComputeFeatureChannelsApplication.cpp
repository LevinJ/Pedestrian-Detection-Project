...
void save_channels_to_txtfile(const AbstractChannelsComputer::input_channels_t &channels,
		const boost::filesystem::path &file_path){
	ofstream textfile1;
	string strtemp= file_path.string() + + ".txt";
	textfile1.open ( strtemp.c_str());
	ofstream textfile2;
	strtemp=file_path.string() + "_dct.txt";
	textfile2.open ( strtemp.c_str());
	...
	for(size_t c = 0; c < num_channels; c += 1 )
	{
		for(size_t y = 0; y < channel_size_y; y += 1 )
		{
			for(size_t x = 0; x < channel_size_x; x += 1 )
			{
				...
				if(c<num_channels/2){
					//original 10 channels
					image_y = c*channel_size_y + y;
					char buff[300];
					sprintf(buff, "x=%d,y=%d, value = %u",image_x,image_y,channels[c][y][x]);
					std::string buffAsStdStr = buff;
					textfile1 << buffAsStdStr << endl;

					if(x==72 && image_y==1352){
					...
					//10 dct channels
					image_y = (c-num_channels/2)*channel_size_y + y;
					char buff[300];
					sprintf(buff, "x=%d,y=%d, value = %u",image_x,image_y,channels[c][y][x]);
					std::string buffAsStdStr = buff;
					textfile2 << buffAsStdStr << endl;
					if(x==72 && image_y==1352){
						printf("%s\n",buff);
						printf("let's check it\n");
					}
		...
		const boost::filesystem::path &file_path)
{
	...
	channel_image_t channels_image(channel_size_x, (channel_size_y*num_channels)/2);
	channel_image_t::view_t channels_image_view = gil::view(channels_image);

	channel_image_t channels_image2(channel_size_x, (channel_size_y*num_channels)/2);
	channel_image_t::view_t channels_image_view2 = gil::view(channels_image2);
	...
	for(size_t c = 0; c < num_channels; c += 1 )
	{
		for(size_t y = 0; y < channel_size_y; y += 1 )
		{
			for(size_t x = 0; x < channel_size_x; x += 1 )
			{
				...
				if(c<num_channels/2){
					//original 10 channels
					image_y = c*channel_size_x + y;
					channels_image_view(image_x, image_y)[0] = channels[c][y][x];
				}
				else{
					//10 dct channels
					image_y = (c-num_channels/2)*channel_size_x + y;
					channels_image_view2(image_x, image_y)[0] = channels[c][y][x];
				}
	...
	gil::png_write_view(file_path.parent_path().string()+ "/" + file_path.stem().string() + "_dct.png",
			gil::const_view(channels_image2));
	log.debug() << "Created geodesic channels file " << file_path << std::endl;
	//wirte the dct channel to text file
	//	writeToTextFile(file_path.parent_path().string()+ "/" + file_path.stem().string() + "_dct.txt"
	//			,float *ImgDctF, int ImgDctFStride,int width,int height);
...
void ComputeFeatureChannelsApplication::main_loop()
{
	...
	while(video_input_is_available and (not end_of_game))
	{
		...
		const AbstractChannelsComputer::input_channels_t &channels = channels_computer_p->get_input_channels_uint8();
		save_channels_to_txtfile(channels,
				output_filename);
