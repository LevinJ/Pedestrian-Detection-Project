...
#include <cstdio> // just for debugging
#include <stdexcept> // just for debugging
...
public:
  ...
  */
  ...
  void setPitch(size_t pitch)
  {
    ...
    if(true and (pitch % sizeof(Type) != 0))
    {
        printf("Layout<>::setPitch pitch == %zi\n", pitch);
        printf("sizeof(Type)  == %zi\n", sizeof(Type) );
        throw std::invalid_argument("assert(pitch % sizeof(Type) == 0) failed");
    }
